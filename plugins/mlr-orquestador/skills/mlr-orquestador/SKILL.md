...
- **Diagnostico de una base de Odoo** (auditoria, revision de salud, estado real de inventario, valuacion, contabilidad, migraciones o codigo a medida), con o sin cotizacion despues → `mlr-diagnostico`. Solo lectura, rondas desde cero (minimo 4, maximo 10) hasta dos seguidas sin hallazgos relevantes, y entregable con capturas via `mlr-redaccion`.
